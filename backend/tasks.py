...
def process_document_task(self, document_id: int, file_path: str, document_type: str):
    ...
    """
    try:
        print(f"📥 [Celery] Starting document analysis: doc_id={document_id}")
...
def process_batch_task(self, batch_id: int, documents_info: list, document_type: str):
    ...
    """
    try:
        print(f"📥 [Celery] Starting batch analysis: batch_id={batch_id}, {len(documents_info)} documents")
        process_batch(batch_id, documents_info, document_type)
